...
if ($actions == 'create_photo_gallery')
 {
    ...
    for($i=0;$i<count($_FILES["image"]["name"]);$i++)
    {
        ...
        $folder ="../../../img/photo_gallery/".$image_name.".jpg";
        $insert_res = $con_obj->insert('photo_gallery', array('image' => $image_path));
...
if ($actions == 'delete_photo_gallery') 
{
    $del_id = $_POST['id'];
    $image_name='';
    $fetch_res = $con_obj->select('photo_gallery', 'image', NULL, "id='$del_id'", NULL);
    $res = $con_obj->numRows();
    if ($res > 0) {
        $res = $con_obj->getResult();
    } else {
    ...
    $filename ="../../../img/photo_gallery/".$str;
  ...
  else 
  {
    ...
    // exit; 
    $delete_res = $con_obj->delete("photo_gallery", "id='$del_id'");
    $con_obj->closeConnection();
    echo $delete_res;
}